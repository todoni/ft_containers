...
protected:
    static std::Allocator<T> static_allocator;
    iterator start;
    iterator finish;
...
public:
    ...
    ~vector() { 
	static_allocator.destroy(start);
	static_allocator.deallocate(start, finish);
    }
...
template <class T>
typename vector<T>::vector_allocator vector<T>::static_allocator;
...
void vector<T>::insert_aux(iterator position, const T& x) {
	...
	*position = x;
	...
	static_allocator.destroy(begin());
	static_allocator.deallocate(begin(), end());
